...
def find_freerouting_jar() -> Optional[Path]:
    ...
    if os.name == "nt":
        ...
        kicad_plugin_paths.append(
            Path(program_files) / "KiCad" / "share" / "kicad" / "scripting" / "plugins"
        )
...
'''
    ...
    try:
        result = subprocess.run(
            [kicad_python, "-c", script], capture_output=True, text=True, timeout=60
        )
...
def run_freerouting(
    ...
    cmd = [
        ...
        jar_path,
        "-de",
        dsn_path,  # Design input
        "-do",
        ses_path,  # Design output (session file)
        "-mp",
        str(max_passes) if max_passes else "100",  # Max passes
        "-host",
        "KiCad-MCP",
    ]
    ...
    try:
        ...
        return {"success": False, "error": f"Freerouting timed out after {timeout} seconds"}
    except Exception as e:
...
def register_freerouting_tools(mcp):
    ...
    async def freeroute_pcb(
        ...
        try:
            ...
            if not import_ses(pcb_path, ses_path):
                return {"error": "Failed to import SES file into PCB", "freerouting_result": result}
    ...
    async def check_freerouting_installation() -> Dict[str, Any]:
        ...
        if jar_path:
            ...
            result["freerouting"]["help"] = (
                "Install via KiCad Plugin Manager (Ctrl+M) or download from https://github.com/freerouting/freerouting"
            )
        ...
        if result["ready"]:
            ...
            missing = [
                k for k, v in result.items() if isinstance(v, dict) and not v.get("installed")
            ]
    ...
    async def import_ses_file(project_path: str, ses_path: str) -> Dict[str, Any]:
        ...
        if import_ses(pcb_path, ses_path):
            return {"success": True, "message": "Imported SES routing into PCB"}
        else:
            return {"error": "Failed to import SES file"}

    logger.info(
        "Registered Freerouting tools: freeroute_pcb, check_freerouting_installation, export_dsn_file, import_ses_file"
    )
