...
class ComponentInfo:
    ...
    def overlaps(self, other: "ComponentInfo", clearance: float = 0.127) -> bool:
        ...
        # Add clearance
        return not (
            b1[2] + clearance < b2[0]  # self right < other left
            or b1[0] - clearance > b2[2]  # self left > other right
            or b1[3] + clearance < b2[1]  # self top < other bottom
            or b1[1] - clearance > b2[3]  # self bottom > other top
        )
...
class SimulatedAnnealingPlacer:
    ...
    def _generate_move(
        ...
        move_type = random.choices(["translate", "swap", "rotate"], weights=[0.6, 0.25, 0.15])[0]
    ...
    def optimize(
        ...
        best_positions = {ref: (c.x, c.y, c.angle) for ref, c in components.items()}
        ...
        return {
            ...
            "improvement_percent": (score_history[0] - final_score) / score_history[0] * 100
            if score_history[0] > 0
            else 0,
...
def extract_placement_from_parsed_pcb(
    ...
    board_info = BoardInfo(width=board_width, height=board_height, margin=1.0)
    ...
    for fp in pcb.footprints:
        ...
        if actual_sizes and ref in actual_sizes.get("components", {}):
            ...
            for pad_start in pad_starts:
                ...
                pads.append(
                    PadInfo(name=pad_name, rel_x=pad_rel_x, rel_y=pad_rel_y, net_name=net_name)
                )
...
def estimate_footprint_size(footprint_lib: str) -> tuple[float, float]:
    ...
    qfn_match = re.search(
        r"(QFN|QFP|LQFP|TQFP).*?(\d+).*?(\d+\.?\d*)x(\d+\.?\d*)", footprint_lib, re.IGNORECASE
    )
    if qfn_match:
...
def extract_placement_from_kicad(
    ...
    for fp in board.GetFootprints():
        ...
        for pad in fp.Pads():
            ...
            pads.append(PadInfo(name=pad.GetName(), rel_x=rel_x, rel_y=rel_y, net_name=net_name))
...
Tip: Run Freerouting now for auto-routing with improved placement."""
    ...
    async def analyze_placement_quality(project_path: str) -> str:
        ...
        try:
            ...
            if not components:
                return (
                    "Error: No components found in PCB. Did you run Update PCB from Schematic (F8)?"
                )
...
"""
    ...
    async def quick_placement_optimization(project_path: str) -> str:
        ...
        try:
            results = optimize_placement(
                project_path=project_path, iterations=1000, initial_temp=50.0, cooling_rate=0.92
            )
...
Saved to: {results["pcb_path"]}"""
    ...
    async def suggest_placement_fixes(project_path: str) -> str:
        ...
        try:
            ...
            # Categorize nets
            power_nets = [
                n for n in net_analysis if n["name"] in ["GND", "VCC", "V1V8", "3V3", "5V", "VBAT"]
            ]
            ...
            for sn in signal_nets[:10]:
                ...
                if sn["furthest_pair"]:
                    ...
                    report.append(
                        f"  Furthest: {p1['comp']}.{p1['pad']} ↔ {p2['comp']}.{p2['pad']}"
                    )
