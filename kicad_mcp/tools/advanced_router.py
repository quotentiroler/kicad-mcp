...
def is_point_in_bounds(
    ...
    return min_x + margin <= p.x <= max_x - margin and min_y + margin <= p.y <= max_y - margin
...
def find_route(
    ...
    while open_set and iterations < max_iterations:
        ...
        if stall_count > 1000:
            logger.warning(
                f"A* stalled at distance {best_distance:.2f}mm after {iterations} iterations"
            )
            return None
        ...
        if dist_to_goal <= goal_tolerance and current.layer == goal.layer:
            ...
            if current.pos != goal.pos:
                ...
                clearance = check_segment_clearance(
                    final_seg, goal.layer, ctx.trace_width, ctx, net_id
                )
        ...
        for neighbor, cost, segment in get_neighbors_45deg(
            current, ctx.grid_step, ctx, goal, net_id, start_pos=start_pos, end_pos=end_pos
        ):
...
def try_direct_route(
    ...
    if check_segment_clearance(seg, layer, ctx.trace_width, ctx, net_id) >= 0:
        return [(RouteNode(start, layer), None), (RouteNode(end, layer), seg)]
    return None
...
def try_l_route(
    ...
    if (
        check_segment_clearance(seg1a, layer, ctx.trace_width, ctx, net_id) >= 0
        and check_segment_clearance(seg1b, layer, ctx.trace_width, ctx, net_id) >= 0
    ):
        return [
    ...
    if (
        check_segment_clearance(seg2a, layer, ctx.trace_width, ctx, net_id) >= 0
        and check_segment_clearance(seg2b, layer, ctx.trace_width, ctx, net_id) >= 0
    ):
        return [
            (RouteNode(start, layer), None),
...
def try_z_route(
    ...
    if abs(dx) > abs(dy):
        ...
        if (
            check_segment_clearance(seg1, layer, ctx.trace_width, ctx, net_id) >= 0
            and check_segment_clearance(seg2, layer, ctx.trace_width, ctx, net_id) >= 0
            and check_segment_clearance(seg3, layer, ctx.trace_width, ctx, net_id) >= 0
        ):
            return [
                (RouteNode(start, layer), None),
...
class ObstacleGrid:
    ...
    def cell_to_pos(self, col: int, row: int) -> Point2D:
        ...
        return Point2D(self.min_x + col * self.grid_step, self.min_y + row * self.grid_step)
...
def jps_find_route(
    ...
    while open_set and iterations < max_iterations:
        ...
        if current == end_cell:
            ...
            if iterations > 100:
                logger.info(
                    f"JPS found route in {iterations} iterations (vs ~{iterations * 10}+ for A*)"
                )
            return route
...
def fast_route_jps(
    ...
    board_area = (ctx.board_bounds[2] - ctx.board_bounds[0]) * (
        ctx.board_bounds[3] - ctx.board_bounds[1]
    )
...
def fast_route_jps_with_layer_change(
    ...
    for t in [0.15, 0.3, 0.7, 0.85]:
        ...
        via_candidates.append(
            Point2D(snap_to_grid(x, ctx.grid_step), snap_to_grid(y, ctx.grid_step))
        )
...
def simplify_path(
    ...
    while i < len(path) - 1:
        ...
        end_node, _ = path[j]
        new_segment = (
            Segment(current_node.pos, end_node.pos) if current_node.pos != end_node.pos else None
        )
...
def build_routing_context(
    ...
    for pad in pads_data:
        ...
        for layer in layers_to_add:
            obstacles[layer].append(
                Obstacle(geometry=rect, layer=layer, net_id=net_id, obstacle_type="pad")
            )
    ...
    for track in pcb_data.tracks:
        ...
        obstacles[track.layer].append(
            Obstacle(geometry=rect, layer=track.layer, net_id=net_id, obstacle_type="trace")
        )
    ...
    for via in pcb_data.vias:
        ...
        for layer in ["F.Cu", "B.Cu"]:
            obstacles[layer].append(
                Obstacle(geometry=circle, layer=layer, net_id=net_id, obstacle_type="via")
            )
    ...
    for seg in segments:
        if seg.get("type") == "via":
            ...
            if layer not in ctx.obstacles:
                ctx.obstacles[layer] = []
            ctx.obstacles[layer].append(
                Obstacle(geometry=rect, layer=layer, net_id=net_id, obstacle_type="trace")
            )
...
def route_single_net(
    ...
    while len(connected) < len(pad_positions):
        ...
        if best_route is None:
            logger.warning(
                f"Could not route all pads for {net_name}, connected {len(connected)}/{len(pad_positions)}"
            )
            break
        ...
        for i in range(len(simplified) - 1):
            ...
            if seg is not None:  # Regular trace
                ...
                all_segments.append(
                    {"type": "via", "position": node1.pos.as_tuple(), "net_id": net_id}
                )
...
def analyze_board_complexity(pcb_data: Any, pads_data: List[Any]) -> BoardAnalysis:
    ...
    routable_nets = sum(1 for n, c in net_pad_counts.items() if c >= 2 and n.upper() != "GND")
    ...
    congestion_factor = (
        min(1.0, total_wire_area / available_routing_area) if available_routing_area > 0 else 1.0
    )
