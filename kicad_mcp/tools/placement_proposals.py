...
class BoundingBox:
    ...
    def expanded(self, margin: float) -> "BoundingBox":
        return BoundingBox(
            self.left - margin, self.top - margin, self.right + margin, self.bottom + margin
        )
...
class ComponentInfo:
    ...
    def get_bbox_at(self, x: float, y: float, rotation: float = None) -> BoundingBox:
        ...
        return BoundingBox(x - half_w, y - half_h, x + half_w, y + half_h)
...
class PlacementState:
    ...
    def check_overlap(
        self, ref: str, x: float, y: float, rotation: float = None, clearance: float = 0.127
    ) -> bool:
    ...
    def is_within_board(
        self, ref: str, x: float, y: float, rotation: float = None, margin: float = 0.5
    ) -> bool:
        ...
        return (
            inner_board.left <= new_bbox.left
            and new_bbox.right <= inner_board.right
            and inner_board.top <= new_bbox.top
            and new_bbox.bottom <= inner_board.bottom
        )
...
class PlacementAnalyzer:
            ...
            for pad in comp.pads:
                ...
                if net_name not in nets:
                    ...
                    nets[net_name] = NetInfo(
                        name=net_name, pad_refs=[], is_power=is_power, is_ground=is_ground
                    )
    ...
    def create_state(self) -> PlacementState:
        ...
        return PlacementState(components=copy.deepcopy(self.components), board_bbox=self.board_bbox)
...
class PlacementOptimizer:
    ...
    def _valid_position(
        self, state: PlacementState, ref: str, x: float, y: float, rotation: float = None
    ) -> bool:
        """Check if position is valid (within bounds, no overlaps)."""
        return state.is_within_board(ref, x, y, rotation, self.margin) and not state.check_overlap(
            ref, x, y, rotation, self.analyzer.clearance
        )
    ...
    def optimize_functional_groups(self, iterations: int = 3000) -> PlacementState:
        ...
        caps = [
            ref for ref, comp in state.components.items() if comp.is_capacitor and not comp.locked
        ]
        others = [
            ref
            for ref, comp in state.components.items()
...
def propose_layouts(project_path: str, iterations: int = 3000) -> dict[str, Any]:
    ...
    wl_improvement = round(
        (current_scores["wirelength"] - wl_scores["wirelength"])
        / max(current_scores["wirelength"], 1)
        * 100,
        1,
    )
    ...
    proposals.append(
        {
            ...
            "component_moves": [
                ...
                if not comp.locked
                and (
                    abs(comp.x - analyzer.components[ref].x) > 0.1
                    or abs(comp.y - analyzer.components[ref].y) > 0.1
                )
            ],
        }
    )
    ...
    cong_improvement = round(
        (current_scores["congestion"] - cong_scores["congestion"])
        / max(current_scores["congestion"], 1)
        * 100,
        1,
    )
    ...
    grp_improvement = round(
        (grp_scores["grouping"] - current_scores["grouping"])
        / max(current_scores["grouping"], 1)
        * 100,
        1,
    )
