...
def _register_pcb_layout_tools(mcp: FastMCP) -> None:
    ...
    @mcp.tool()
    def place_component(
        project_path: str, reference: str, x: float, y: float, rotation: float = 0.0
    ) -> dict[str, Any]:
    ...
    @mcp.tool()
    def place_multiple_components(project_path: str, placements: list[dict]) -> dict[str, Any]:
        ...
        for placement in placements:
            ...
            try:
                success = update_footprint_position(
                    pcb_path=pcb_path, reference=ref, new_position=(abs_x, abs_y), new_rotation=rot
                )
                ...
                if success:
                    ...
                    results["failed"].append(
                        {"reference": ref, "reason": "Component not found in PCB"}
                    )
            except Exception as e:
                results["failed"].append({"reference": ref, "reason": str(e)})
    ...
    def add_trace(
        ...
        try:
            ...
            track = Track(
                start=(start_x, start_y), end=(end_x, end_y), width=width, layer=layer, net=net_id
            )
    ...
    # @mcp.tool()  # Disabled - use KiCad GUI instead
    def add_via(
        project_path: str, x: float, y: float, net_name: str, size: float = 0.8, drill: float = 0.4
    ) -> dict[str, Any]:
        ...
        try:
            ...
            via = Via(position=(x, y), size=size, drill=drill, net=net_id)
    ...
    @mcp.tool()
    def suggest_component_placement(project_path: str) -> dict[str, Any]:
        ...
        try:
            ...
            if categories["mcu"]:
                suggestions["groups"]["mcu"] = {
                    "zone": {
                        "x": board_width * 0.4,
                        "y": board_height * 0.5,
                        "description": "Center of board",
                    },
                    "components": [
                        {
                            "reference": c["reference"],
                            "suggested_x": board_width * 0.4,
                            "suggested_y": board_height * 0.5,
                        }
            ...
            if categories["power"]:
                suggestions["groups"]["power"] = {
                    "zone": {
                        "x": board_width * 0.15,
                        "y": board_height * 0.3,
                        "description": "Top-left, near power input",
                    },
                    "components": [
                        {
                            "reference": c["reference"],
                            "suggested_x": board_width * 0.1 + i * 3,
                            "suggested_y": board_height * 0.3,
                        }
            ...
            if categories["sensors"]:
                suggestions["groups"]["sensors"] = {
                    "zone": {
                        "x": board_width * 0.5,
                        "y": board_height * 0.8,
                        "description": "Bottom center, facing skin",
                    },
                    "components": [
                        {
                            "reference": c["reference"],
                            "suggested_x": board_width * 0.3 + i * 8,
                            "suggested_y": board_height * 0.8,
                        }
            ...
            if categories["rf"]:
                suggestions["groups"]["rf"] = {
                    "zone": {
                        "x": board_width * 0.85,
                        "y": board_height * 0.5,
                        "description": "Right edge, antenna keepout",
                    },
                    "components": [
                        {
                            "reference": c["reference"],
                            "suggested_x": board_width * 0.85,
                            "suggested_y": board_height * 0.3 + i * 3,
                        }
            ...
            if categories["connectors"]:
                suggestions["groups"]["connectors"] = {
                    ...
                    "components": [
                        {
                            "reference": c["reference"],
                            "suggested_x": board_width * 0.95,
                            "suggested_y": board_height * 0.4 + i * 4,
                        }
            ...
            if categories["passives"]:
                suggestions["groups"]["passives"] = {
                    "zone": {
                        "description": "Near associated ICs - place decoupling caps within 2mm of IC power pins"
                    },
    ...
    def auto_place_decoupling_caps(
        project_path: str, ic_reference: str, cap_distance: float = 1.5
    ) -> dict[str, Any]:
