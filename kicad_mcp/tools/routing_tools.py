...
def register_routing_tools(mcp):
    ...
    def auto_route_net(
        ...
        if not ADVANCED_ROUTER_AVAILABLE:
            return {
                "error": "Advanced router not available. Consider using Freerouting for auto-routing."
            }
    ...
    def add_ground_pour(
        ...
        try:
            ...
            zone_sexpr = create_zone_sexpr(
                net_name=net_name, net_id=net_id, layer=layer, outline=outline, priority=0
            )
    ...
    def route_all_nets(
        ...
        if not ADVANCED_ROUTER_AVAILABLE:
            return {
                "error": "Advanced router not available. Consider using Freerouting (freeroute_pcb) for auto-routing."
            }
        ...
        try:
            ...
            results = {"routed": [], "failed": [], "skipped": list(skip_nets)}
            ...
            for i, net_name in enumerate(net_cats.get("power", [])):
                ...
                if segments:
                    ...
                    results["routed"].append(
                        {"net": net_name, "traces": len(segments), "type": "power"}
                    )
                    pcb = parse_pcb_file(pcb_path)
                else:
            ...
            for i, net_name in enumerate(net_cats.get("signal", [])):
                ...
                if segments:
                    ...
                    update_context_with_segments(ctx, segments)
                    results["routed"].append(
                        {"net": net_name, "traces": len(segments), "type": "signal"}
                    )
                    pcb = parse_pcb_file(pcb_path)
                else:
                    results["failed"].append(net_name)
    ...
    def analyze_board_for_routing(project_path: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "layer_explanation": _get_layer_explanation(
                    analysis.layer_recommendation, analysis
                ),
    ...
    def _get_layer_explanation(layers: int, analysis) -> str:
        ...
        if layers == 2:
            return (
                f"2 layers sufficient: Low congestion ({analysis.congestion_factor:.0%}), "
                f"moderate density ({analysis.pad_density:.3f} pads/mm²). "
                "Top for signals, bottom for ground pour."
            )
        elif layers == 4:
            return (
                f"4 layers recommended: Moderate congestion ({analysis.congestion_factor:.0%}). "
                "Typical stack: Signal-GND-Power-Signal. "
                "Inner planes provide shielding and power distribution."
            )
        else:
            return (
                f"6+ layers needed: High congestion ({analysis.congestion_factor:.0%}), "
        ...
        try:
            ...
            for net_name, pad_list in net_pads.items():
                ...
                if net_name in routed_nets:
                    ...
                    unrouted.append(
                        {"net": net_name, "pads": len(pad_list), "pad_list": pad_list[:5]}
                    )
        ...
        try:
            ...
            for pad in pads:
                ...
                for dx in range(-int((w / 2 + clearance) * 2), int((w / 2 + clearance) * 2) + 1):
                    for dy in range(
                        -int((h / 2 + clearance) * 2), int((h / 2 + clearance) * 2) + 1
                    ):
        ...
        for fp in f_points:
            for bp in b_points:
                ...
                if dist < 0.5:  # Close enough to need a via
                    ...
                    if via_pos_rounded not in existing_vias and not point_hits_any_pad(
                        via_pos[0], via_pos[1], net_name
                    ):
...
def find_dangling_tracks(pcb: PCBData, pads: list[Pad]) -> list[str]:
    ...
    for track in pcb.tracks:
        ...
        start_connections = sum(
            1 for pos, uid in track_endpoints[track.layer] if pos == start and uid != track.uuid
        )
        end_connections = sum(
            1 for pos, uid in track_endpoints[track.layer] if pos == end and uid != track.uuid
        )
...
def clear_all_tracks_from_pcb(pcb_path: str, keep_zones: bool = True) -> dict[str, Any]:
    ...
    if original_opens != original_closes:
        logger.warning(
            f"PCB file already unbalanced: {original_opens} opens, {original_closes} closes"
        )
    ...
    if new_opens != new_closes:
        logger.error(
            f"PCB file became unbalanced after modification: {new_opens} opens, {new_closes} closes"
        )
        return {
            "success": False,
...
def register_routing_fix_tools(mcp):
    ...
    logger.info(
        "Registered routing fix tools: add_layer_transition_vias, remove_dangling_tracks, clear_all_tracks"
    )

    logger.info(
        "Registered routing tools: auto_route_net, add_ground_pour, route_all_nets, get_unrouted_nets, add_via_stitching"
    )
